...
#ts for accuracy test visualization
...
# Forcast just first two month of 2010
ETS_model_month <-  stlf(ts_daily,h=59)
...
#NN_fit <- nnetar(ts_daily,p=1,P=1)
NN_model <- nnetar(ts_daily,decay=7, maxit=150, p=1,P=0,xreg=fourier(ts_daily, K=c(3,2)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_model, h=365,xreg=fourier(ts_daily, 
                                          K=c(3,2),h=365))
#NN_for <- forecast(NN_fit, h=59) 
NN_for_month <- forecast(NN_model, h=59,xreg=fourier(ts_daily, K=c(3,2) ,h=59))
...
NN_model2010 <- nnetar(ts_daily2010,decay=7, maxit=150,p=1,P=0,xreg=fourier(ts_daily2010, K=c(3,2)))
...
#Create data frame using the accuracy test score for only the first two month in 2010
seas_scores <- as.data.frame(rbind(ETS_scores_for, TBATS_scores_for, ARIMA_Four_scores_for, NN_scores1_for))
...
According to the comparison matrics the best model is the neural Network model
