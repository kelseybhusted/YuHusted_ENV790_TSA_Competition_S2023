...
Hourly data was transformed into daily data with aggregate functions and pipes (i.e., tidyvers).
...
#Forecast
ARIMA_Four_for2011 <- forecast(ARIMA_Four_model,
                           xreg=fourier(ts_daily2010,
                                        K=c(2,12),
                                        h=59),
                           h=59
...
NN_model <- nnetar(ts_daily,p=1,P=0,xreg=fourier(ts_daily, K=c(2,16)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_model, h=365,xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,16),h=365))
...
#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN_model2010 <- nnetar(ts_daily2010,p=1,P=0,xreg=fourier(ts_daily2010, K=c(2,16)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for2010 <- forecast(NN_model2010, h=59,xreg=fourier(ts_daily, 
                                          K=c(2,16),h=59))
